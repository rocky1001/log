...
import (
	...
	rotatewriter "github.com/natefinch/lumberjack"
)
...
const (
	defaultLogFile     = "./run/collector.log"
	defaultReserveCnt  = 5
	defaultRotateSize  = 10 //MB
	defaultReserveDays = 10
)
...
type Logger struct {
	...
	flag       int        // properties
	Level      int
	out        rotatewriter.Logger // destination for output
...
func New(logFile string, maxSize, maxAge, maxBackups, level, flag int, prefix string) {
	if logFile == "" {
	...
	if maxAge == 0 {
		maxAge = defaultReserveDays
	}
	if maxBackups == 0 {
		maxBackups = defaultReserveCnt
	}
	if flag == 0 {
		flag = Ldefault
	}
...
func (l *Logger) Output(reqId string, lvl int, calldepth int, s string) error {
	if Std == nil {
		return fmt.Errorf("invalid logger, need init")
	}
	if lvl < l.Level {
		return nil
	}
